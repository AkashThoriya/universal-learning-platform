      ...
      subject.topics.forEach(topic => {
        ...
        if (effectiveProgress?.needsReview) {
          items.push({
            ...
            status:
              effectiveProgress.masteryScore >= 80
                ? 'mastered'
                : effectiveProgress.masteryScore > 0
                  ? 'in_progress'
                  : 'not_started',
            practiceCount: 0,
            ...
            <Tabs value={filter} onValueChange={v => setFilter(v as typeof filter)} className="w-full">
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="overdue" className="text-xs sm:text-sm">
                  🔴 Overdue (
                  {reviewItems.filter(i => i.nextRevision && isPast(i.nextRevision) && !isToday(i.nextRevision)).length}
                  )
                </TabsTrigger>
              </TabsList>
                          ...
                          <CardContent className="space-y-3">
                            ...
                            <div className="flex gap-2 pt-2">
                              ...
                              <Button size="sm" className="flex-1" asChild>
                                <Link
                                  href={`/syllabus/${item.type === 'subtopic' ? item.topicId : item.id}?subject=${item.subjectId}`}
                                >

      ...
      try {
        ...
        if (fetchedUser) {
          ...
          const formData: ProfileFormData = {
            ...
            examDate: fetchedUser.currentExam?.targetDate
              ? (fetchedUser.currentExam.targetDate.toDate().toISOString().split('T')[0] ?? '')
              : '',
            preparationStartDate: fetchedUser.preparationStartDate
              ? (fetchedUser.preparationStartDate.toDate().toISOString().split('T')[0] ?? '')
              : '',
            ...
            preferences: {
              ...
              tierDefinitions: { ...DEFAULT_PREFERENCES.TIER_DEFINITIONS } as any,
              revisionIntervals: fetchedUser.preferences?.revisionIntervals ?? [
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const updateData: Partial<UserType> = {
        ...
        ...(form.data.preparationStartDate
          ? { preparationStartDate: Timestamp.fromDate(new Date(form.data.preparationStartDate)) }
          : {}),
        preferences: {
              ...
              <div className="space-y-6">
                        ...
                        return (
                          ...
                          >
                            <CardContent className="p-5 text-center relative z-10">
                              ...
                              <div
                                className={`mb-3 inline-flex p-3 rounded-full ${isSelected ? 'bg-background' : 'bg-muted/50'}`}
                              >
                                {type === 'student' && (
                                  <User
                          ...
                          <div className="grid gap-4">
                            {form.data.selectedCourses
                              ...
                              .map(course => (
                                <div
                                  key={course.examId}
                                  className={`p-4 rounded-lg border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${course.isPrimary ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}
                                >
                                  <div>
                                    <div className="flex items-center space-x-2">
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">
                                      Target:{' '}
                                      {course.targetDate &&
                                        ((course.targetDate as any).toDate
                                          ? (course.targetDate as any).toDate().toLocaleDateString()
                                          : new Date(course.targetDate as any).toLocaleDateString())}
                                    </p>
                                  </div>
                                    ...
                                    {!course.isPrimary && (
                                      <>
                                        <Button
                                          size="sm"
                                          variant="ghost"
                                          onClick={() => handleSetPrimaryCourse(course.examId)}
                                        >
                                          Set Primary
                                        </Button>
                                        <Button
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        ...
                        {validationErrors.examDate && (
                          <p className="text-sm text-red-600">{validationErrors.examDate}</p>
                        )}
                ...
                <TabsContent value="syllabus" className="space-y-6">
                  ...
                  <div className="flex flex-col space-y-2">
                    ...
                    <p className="text-muted-foreground text-sm">Manage your subjects for this course</p>
                  </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {form.data.syllabus.map(subject => (
                          <Card
                            key={subject.id}
                            className="bg-background shadow-sm hover:shadow-md transition-shadow border ring-1 ring-black/5"
                          >
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                  {subject.isCustom ? (
                                    ...
                                    <h4 className="font-semibold text-sm truncate pr-2" title={subject.name}>
                                      {subject.name}
                                    </h4>
                                  )}
                                  {subject.topics && (
                        ...
                        <div className="flex items-center space-x-3 overflow-x-auto pb-2 pt-4">
                          {form.data.preferences.revisionIntervals.map((interval, index) => (
                            <div key={index} className="flex items-center space-x-2 flex-shrink-0">
                              <div className="relative">
                                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-muted text-[10px] px-1.5 py-0.5 rounded-full border whitespace-nowrap">
                                  R{index + 1}
                                </span>
                                <Input
                                  type="number"

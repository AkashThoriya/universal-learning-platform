                    ...
                    .map((conceptStr, idx) => {
                      ...
                      return (
                        <div key={idx} className={cn("pl-4 border-l-2", borderColor)}>
                           ...
                           <p className={cn("leading-relaxed text-base font-medium max-w-prose", category ? "text-slate-800" : "text-slate-700")}>
                             {content}
                           </p>
                        </div>
                      );
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {topic.practiceQuestions && topic.practiceQuestions.length > 0 && (
                <p className="text-sm font-medium text-slate-500 bg-slate-100 px-3 py-1 rounded-full">
                  {progress?.solvedQuestions?.length || 0} <span className="text-slate-400">/</span> {topic.practiceQuestions?.length || 0} Solved
                </p>
              )}
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        >
                          <CheckCircle size={14} fill="currentColor" className={cn("transition-transform duration-300", isSolved ? "scale-100" : "scale-0")} />
                        </button>
                        
                        <div>
                          <div className={cn("font-medium transition-colors duration-300 text-base", isSolved ? "text-slate-500 line-through decoration-slate-300" : "text-slate-900 group-hover:text-blue-700")}>
                            {question.name}
                          </div>
                          <div className="flex gap-2 mt-1.5">
                            <Badge 
                              ...
                              className={cn(
                                ...
                                question.difficulty === 'Easy' ? "bg-green-50 border-green-100 text-green-700" :
              ...
              <div className="py-16 px-6 bg-slate-50/50 rounded-xl border border-dashed border-slate-200 text-center relative overflow-hidden">
                <div className="relative z-10 max-w-md mx-auto space-y-4">
                  ...
                  <p className="text-slate-600 leading-relaxed">
                    This topic focuses on foundational knowledge. There are no coding problems here—your goal is to understand the core principles thoroughly.
                  </p>
                  <div className="pt-4">
                    <Button variant="outline" className="border-slate-200 text-slate-700 bg-white hover:bg-slate-50" disabled>
...
function TabTriggerItem({ value, icon, label, count }: { value: string; icon: React.ReactNode; label: string; count?: number }) {
  return (
    ...
    >
      <div className="group-data-[state=active]:text-blue-600 transition-colors">
        {icon}
      </div>
      <span className="font-medium">{label}</span>
      {count !== undefined && count > 0 && (
         <span className="ml-1 text-[10px] py-0.5 px-1.5 bg-slate-100 text-slate-600 font-bold rounded-full border border-slate-200 group-data-[state=active]:bg-blue-50 group-data-[state=active]:text-blue-700 group-data-[state=active]:border-blue-100">{count}</span>
      )}
    </TabsTrigger>
  );
}

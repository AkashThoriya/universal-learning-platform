...
export default function StrategyInsights({ user, syllabus, completedTopicsCount }: StrategyInsightsProps) {
  ...
  if (!user.preparationStartDate) {
    return (
      <Card className="mb-6 border-l-4 border-l-indigo-500 bg-gradient-to-br from-indigo-50 to-white shadow-md overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-1">Set your <strong className="text-indigo-600">Preparation Start Date</strong> to activate velocity tracking, pace analysis, and projected finish dates.</p>
              </div>
            </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      <Card className="lg:col-span-2 shadow-md border-indigo-100 bg-gradient-to-br from-white to-indigo-50/30">
        ...
        <CardContent className="space-y-6">
            ...
            <div className="relative pt-6 pb-2">
                <div className="flex justify-between text-sm font-medium text-muted-foreground mb-2">
                    <span className="flex items-center gap-1"><Calendar className="h-3 w-3"/> Start: {formatDate(metrics.startDate)}</span>
                    <span className="flex items-center gap-1"><Target className="h-3 w-3"/> Exam: {formatDate(metrics.examDate)}</span>
                </div>
                ...
                <div className="space-y-3">
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs">
                            <span className="text-indigo-600 font-medium">Content Completion</span>
                            <span className="text-indigo-600">{metrics.percentageContentCompleted.toFixed(0)}%</span>
                        </div>
                        <Progress value={metrics.percentageContentCompleted} className="h-2 bg-indigo-100" indicatorClassName="bg-indigo-600" />
                    </div>
                    <div className="space-y-1">
                         <div className="flex justify-between text-xs">
                        ...
                        <Progress value={metrics.percentageTimeElapsed} className="h-1.5 bg-gray-100" indicatorClassName="bg-gray-400" />
                    </div>
                </div>
            </div>

            {/* Velocity Comparison */}
            <div className="grid grid-cols-2 gap-4 bg-white/60 rounded-xl p-4 border border-indigo-50">
                <div>
                 ...
                 <div>
                    ...
                    <div className="flex items-baseline gap-1 mt-1">
                        <span className={`text-2xl font-bold ${metrics.currentVelocity >= metrics.requiredVelocity ? 'text-green-600' : 'text-amber-600'}`}>
      ...
      {/* Projection Card */}
      <Card className={`shadow-md border-l-4 ${metrics.status === 'on_track' || metrics.status === 'ahead' ? 'border-l-green-500' : 'border-l-amber-500'}`}>
        <CardHeader>
        ...
        <CardContent>
            ...
            {metrics.status === 'critical' && (
                 <div className="mt-6 p-3 bg-red-50 text-red-700 text-xs rounded-lg border border-red-100 flex gap-2">
...
function StatusBadge({ status }: { status: string }) {
    const styles = {
        on_track: "bg-green-100 text-green-700 border-green-200 hover:bg-green-100",
